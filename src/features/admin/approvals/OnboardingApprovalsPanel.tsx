...
type ModalState = { kind: 'approve' | 'reject' | 'send'; id: string; note: string } | null

export function OnboardingApprovalsPanel() {
  ...
  const selected = useMemo(() => filtered.filter((r) => sel[r.id]), [filtered, sel])
  const canBulkApprove = selected.length > 0 && selected.every((r) => r.docs.length === 0) && !busy
    ...
    <>
      <div className="bg-panel border border-border-light rounded-xl p-5 shadow-[0_2px_8px_rgba(0,0,0,.2)] transition-shadow duration-200 hover:shadow-[0_4px_12px_rgba(0,0,0,.3)]">
        <div className="flex gap-[10px] flex-wrap items-center">
          ...
          <button className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)]" onClick={() => setStatus('Pending')}>
            Pending only
          </button>
          <button
            ...
            title={selected.length > 0 && !canBulkApprove ? 'Bulk approve is only enabled when selected rows have no documents.' : undefined}
          >
            {busy ? 'Working…' : 'Approve selected (no-docs)'}
          </button>
        </div>
      </div>
          ...
          <thead>
            <tr>
              <th className="w-[42px] py-3.5 px-4 text-left border-t-0 text-muted text-[11px] font-semibold uppercase tracking-[0.5px] bg-bg-secondary sticky top-0 z-10">
                <input className="w-[18px] h-[18px] accent-accent cursor-pointer" type="checkbox" checked={allChecked} onChange={toggleAll} />
              </th>
              ...
              <th className="py-3.5 px-4 text-right border-t-0 text-muted text-[11px] font-semibold uppercase tracking-[0.5px] bg-bg-secondary sticky top-0 z-10">Review</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="transition-colors duration-150 hover:bg-[rgba(255,255,255,.05)] last:td:border-b-0">
                <td className="py-3.5 px-4 text-left border-t border-border-light">
                  <input className="w-[18px] h-[18px] accent-accent cursor-pointer" type="checkbox" checked={!!sel[r.id]} onChange={() => toggle(r.id)} />
                </td>
                <td className="py-3.5 px-4 text-left border-t border-border-light font-extrabold">{r.id}</td>
        ...
        <div className="fixed inset-0 grid place-items-center z-[60]">
          ...
          <div className="w-[min(560px,92vw)] bg-panel border border-border-light rounded-2xl p-6 shadow-[0_20px_60px_rgba(0,0,0,.5)] relative z-10">
            <div className="flex justify-between items-center gap-[10px]">
              <div className="font-black">{modal.kind === 'approve' ? 'Approve application' : modal.kind === 'reject' ? 'Reject application' : 'Send back for changes'}</div>
              <button className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)]" onClick={() => setModal(null)}>
                Close
              </button>
            </div>
            <div className="h-2.5" />
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-screen w-[min(560px,92vw)] bg-bg-secondary border-l border-border-light p-6 overflow-auto z-50 shadow-[-4px_0_20px_rgba(0,0,0,.3)]">
        <div className="flex items-center justify-between gap-[10px]">
          ...
          <button className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)]" onClick={onClose}>
            Close
          </button>
        </div>
        ...
        <div className="inline-flex gap-1 p-1 border border-border-light rounded-[10px] bg-panel-2">
          <button
            className={clsx(
              ...
              tab === 'info' ? 'text-text bg-accent-light border-[rgba(59,130,246,.3)] font-semibold' : 'hover:text-text hover:bg-[rgba(255,255,255,.04)]',
            )}
            onClick={() => setTab('info')}
          >
            Information
          ...
          <button
            className={clsx(
              'py-2 px-4 rounded-lg cursor-pointer text-text-secondary border border-transparent text-[13px] font-medium transition-all duration-200',
              tab === 'docs' ? 'text-text bg-accent-light border-[rgba(59,130,246,.3)] font-semibold' : 'hover:text-text hover:bg-[rgba(255,255,255,.04)]',
            )}
            onClick={() => setTab('docs')}
          >
              ...
              <div className="grid gap-[10px]">
                ...
                <div className="flex justify-between gap-[10px] flex-wrap">
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <a className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)]" href={cur?.url ?? '#'} target="_blank" rel="noreferrer">
                      Open
                    </a>
                  </div>

                  <label className="inline-flex gap-2 items-center text-xs text-muted leading-normal">
                    <input className="w-[18px] h-[18px] accent-accent cursor-pointer" type="checkbox" checked={!!reviewed[cur?.id ?? '']} onChange={(e) => markReviewed(e.target.checked)} />
        ...
        <div className="bg-panel border border-border-light rounded-xl p-5 shadow-[0_2px_8px_rgba(0,0,0,.2)] transition-shadow duration-200 hover:shadow-[0_4px_12px_rgba(0,0,0,.3)] bg-[rgba(255,255,255,.04)]">
          ...
          <div className="flex gap-2 justify-end flex-wrap">
            <button className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)] disabled:opacity-50 disabled:cursor-not-allowed" onClick={onApprove} disabled={!allReviewed}>
              Approve
            </button>
            <button className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)] disabled:opacity-50 disabled:cursor-not-allowed" onClick={onSend} disabled={!allReviewed}>
              Send back
            </button>
            <button className="bg-panel border border-border text-text py-2 px-3 rounded-lg cursor-pointer text-[13px] font-semibold transition-all duration-200 whitespace-nowrap hover:bg-panel-2 hover:border-border hover:shadow-[0_2px_4px_rgba(0,0,0,.2)] disabled:opacity-50 disabled:cursor-not-allowed" onClick={onReject} disabled={!allReviewed}>
              Reject
            </button>
          </div>
        </div>
